...
class Iocify {
  ...
  _getBeanInstance(beanName, BeanClass, type) {
    if (!type || type === TYPES.NEW_INSTANCE) {
      ...
      if (typeof BeanClass.default === 'function') return new BeanClass.default();
      this._error(beanName, `doesn't have a class`);
    }
  ...
  _injectBeanProperties(beanInstance, beanPropertiesConfig) {
    ...
    return;
    function getPropertyValue(config) {
      if (typeof config === 'object') {
        return Object.keys(config).reduce((result, key) => {
          result[key] = getPropertyValue(config[key]);
          return result;
        }, {});
      }
      const isEnvValue = config.indexOf('$ENV') === 0;
      if (!isEnvValue) return config;
      const envName = config.replace(/\$ENV\.(.*)/, '$1');
